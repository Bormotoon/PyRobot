...
1.  **Рефакторинг `kumir_datatypes.py` и `utils.py`:**
    *   Удалить дублирующее определение `KumirValue` из `utils.py`.
    *   Определить классы `KumirFunction` и `KumirTable` в `kumir_datatypes.py`.
    *   Обновить импорты в `expression_evaluator.py` для использования этих классов из `kumir_datatypes.py`.
2.  **Исправление ошибок в `expression_evaluator.py`:**
    *   Скорректировать относительные импорты.
    *   Исправить доступ к токенам `KumirParser` (например, `KumirParser.MUL` вместо `KumirParser.MULTIPLY`).
    *   Убрать использование `is_error` и `error_message` в `KumirValue`, перейти на исключения.
    *   Исправить доступ к атрибутам контекста (`ctx`) и `TerminalNode`.
    *   Добавить проверки типов перед доступом к специфичным атрибутам `KumirVariable`.
    *   Удалить дублирующиеся определения методов.
    *   Реализовать или правильно импоровать `get_kumir_type_name_from_py_value`.
3.  **Проверка и тестирование `ExpressionEvaluator`.**

---

## 2025-05-25: Реализация вызова функций в ExpressionEvaluator

**План:**
1.  **Реализовать вызов функций в `_evaluate_postfix_expression` в файле `pyrobot/backend/kumir_interpreter/interpreter_components/expression_evaluator.py`**.
    *   Извлечь имя функции из `primary_expr_ctx` (которое является результатом `_evaluate_primary_expression` до цикла по `postfix_parts`). Если `primary_expr_ctx` было идентификатором, то `current_value` на этом этапе будет содержать `KumirValue` с именем переменной (которая должна быть функцией) или сама функция, если `_evaluate_primary_expression` может возвращать дескрипторы функций.
        *   **Коррекция:** `_evaluate_primary_expression` возвращает `KumirValue`. Имя функции должно быть извлечено из `primary_expr_ctx.qualifiedIdentifier().getText()` *до* вычисления `current_value` для первого постфиксного оператора, если этот оператор - вызов функции.
        *   **Уточнение:** `current_value` после `_evaluate_primary_expression` представляет собой значение, к которому применяется постфиксная операция. Если это вызов функции, то `current_value` должно быть самой функцией (или информацией о ней). Если это метод объекта (не применимо к КуМиру напрямую, но для аналогии), то `current_value` - это объект.
        *   **Для КуМира:** `current_value` после `_evaluate_primary_expression(primary_expr_ctx)` будет значением переменной или результатом выражения в скобках. Если это имя функции, то `primary_expr_ctx.qualifiedIdentifier().getText()` даст имя. Затем это имя нужно будет использовать для поиска функции в `self.visitor.scope_manager` или `self.visitor.procedure_manager`.
    *   Получить список аргументов из `part_ctx.argumentList()`. `argumentList` (если есть) содержит `expressionList`, который, в свою очередь, содержит список `expression`. Каждое из этих выражений-аргументов нужно будет вычислить с помощью `self.evaluate()`.
    *   Выполнить вызов функции через `self.visitor` (например, `self.visitor.call_user_function(function_name, evaluated_args, call_site_context)` или аналогичный метод, который может также обрабатывать встроенные функции). Результат вызова функции станет новым `current_value` для следующей итерации постфиксных операций или конечным результатом выражения.
    *   Обработать случай, когда `current_value` (то, что должно быть функцией) не является вызываемым объектом (например, это обычная переменная).
2.  **Обновить `AI_notes.md`** с прогрессом после реализации.

---

## 2025-05-26: Отладка ProcedureManager и KumirDatatypes
