...
from .kumir_interpreter.kumir_exceptions import (
    KumirExecutionError, KumirEvalError,
...
def get_field_state_from_session():
    ...
    if not isinstance(s, dict):
        if s is not None:
            logger.warning(f"Invalid type for field_state in session: "
                          f"{type(s)}. Clearing.")
    ...
    pos_ok = (isinstance(s.get('robotPos'), dict) and 
              isinstance(s['robotPos'].get('x'), int) and 
              isinstance(s['robotPos'].get('y'), int))
    
    if width_ok and height_ok and pos_ok:
        logger.debug("Valid field state retrieved from session.")
        return s
    else:
        logger.warning("Invalid field state content in session. Clearing.")
        session.pop('field_state', None)
        session.modified = True
        return None
...
def update_field():
    ...
    if not (isinstance(robotPos, dict) and 
            isinstance(robotPos.get('x'), int) and 
            isinstance(robotPos.get('y'), int)):
    ...
    if not isinstance(d.get('markers'), dict):
        errors['markers'] = "Must be an object/dict."
    ...
    if not isinstance(d.get('symbols'), dict):
        errors['symbols'] = "Must be an object/dict."
    if not isinstance(d.get('radiation'), dict):
        errors['radiation'] = "Must be an object/dict."
    if not isinstance(d.get('temperature'), dict):
        errors['temperature'] = "Must be an object/dict."

    if errors:
        logger.warning(f"Invalid data received in /updateField: {errors}. "
                      f"Data: {d}")
        return jsonify({
            'success': False,
    ...
    return jsonify({
        'success': True,
        'message': 'Field state updated in session.'
    }), 200
...
def execute_code():
    ...
    logger.info(f"Execute code request received (Session SID: "
               f"{session_id_for_log})")
    
    if not request.is_json:
        logger.warning("Invalid content type for /execute. Expected JSON.")
        return jsonify({
            'success': False,
            'message': 'Invalid request content type (expected JSON).'
        }), 415
    ...
    if not code:
        ...
        return jsonify({
            'success': False,
            'message': 'Код для выполнения пуст.'
        }), 200
    
    initial_state = client_state or get_field_state_from_session()
    state_source = ('Request' if client_state else 
                   ('Session' if session.get('field_state') else 'Default'))
    logger.debug(f"Using initial field state from: {state_source}")
    
    if not initial_state:
        logger.info("No initial state provided or found in session, "
    ...
    try:
        ...
        def progress_callback(progress_data):
            ...
            try:
                eventlet.sleep(0)
                socketio.emit('execution_progress', progress_data, 
                             to=current_sid)
            except Exception as emit_err:
                logger.error(f"Error emitting progress to SID {current_sid}: "
                           f"{emit_err}")

        result = interpreter.interpret(progress_callback=progress_callback)
        
        if result.get('input_required'):
            logger.info(f"Execution requires input for variable "
                       f"'{result.get('var_name')}'. Returning input request.")
        ...
        response_data = {
            ...
            'message': result.get('message', 'OK' if result.get('success') 
                                 else 'Unknown Error'),
            'finalState': final_state_data,
            'trace': trace_data
        }
        
        if not response_data['success']:
            response_data['errorIndex'] = result.get('errorIndex', -1)
            logger.warning(f"Execution finished with error: "
                          f"{response_data['message']} "
                          f"(Error Index: {response_data.get('errorIndex')})")
        else:
            logger.info("Execution completed successfully.")
            try:
                if (interpreter and final_state_data and 
                    final_state_data.get('robot') is not None):
                    field_state_to_save = {
                    ...
                    field_state_to_save = {k: v for k, v in 
                                          field_state_to_save.items() 
                                          if v is not None}
                    session['field_state'] = field_state_to_save
                    session.modified = True
                    logger.debug("Saved final field state to session after "
                                "successful execution.")
                else:
                    logger.warning("Could not save final state to session: "
                                  "interpreter/finalState/robotPos missing.")
            except Exception as save_err:
                logger.error(f"Error saving final state to session: "
                           f"{save_err}", exc_info=True)
        ...
        try:
            state_on_error = (interpreter.get_state() 
                             if interpreter and hasattr(interpreter, 'get_state') 
                             else {})
            state_on_error["output"] = output_on_error
            error_index = getattr(e, 'line_index', -1)
        except Exception as getStateErr:
            logger.error(f"Error getting state after Kumir error: "
                        f"{getStateErr}")
            state_on_error = {"output": output_on_error}
...
def reset_simulator_session():
    session_id_for_log = session.get('sid', 'None')
    logger.info(f"Reset field state request received (Session SID: "
               f"{session_id_for_log}).")
    session.pop('field_state', None)
    session.modified = True
    ...
    return jsonify({
        'success': True,
        'message': 'Состояние поля сброшено в сессии.'
    }), 200
...
if __name__ == '__main__':
    ...
    debug_mode = (os.environ.get('FLASK_DEBUG', 'False').lower() == 'true' or 
                  is_development)
    use_reloader = (os.environ.get('USE_RELOADER', str(is_development))
                   .lower() == 'true')
    ...
    logger.info(f"  Session Secure Cookie: "
