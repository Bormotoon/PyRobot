...
from pyrobot.backend.kumir_interpreter.kumir_exceptions import (
    ...
    KumirTypeError, KumirArgumentError, BreakSignal, ContinueSignal, StopExecutionSignal, ReturnSignal, # Изменено            for i, arg_ctx in enumerate(ctx.ioArgumentList().ioArgument()):
                # print(f"[DEBUG visitIoStatement] Processing OUTPUT arg #{i}: {arg_ctx.getText()}", file=sys.stderr)topSignal на StopExecutionSignal
...
class StatementHandlerMixin(KumirParserVisitor):
    ...
    def visitAssignmentStatement(self, ctx: KumirParser.AssignmentStatementContext) -> None:
        # This mixin expects the following attributes to be available on the class that uses it:
        # self.scope_manager, self.expression_evaluator, self.procedure_manager
        kiv_self = cast('KumirInterpreterVisitor', self)
    ...
    def visitIoStatement(self, ctx: KumirParser.IoStatementContext) -> None:
        ...
        if ctx.OUTPUT(): # Проверяем, что это команда ВЫВОД (OUTPUT из грамматики)
            current_output = ""
            # print(f"[DEBUG visitIoStatement] OUTPUT statement found. Args count: {len(ctx.ioArgumentList().ioArgument())}", file=sys.stderr)
        ...
        elif ctx.INPUT(): # Проверяем, что это команда ВВОД (INPUT из грамматики)
...
class StatementHandler(StatementHandlerMixin):
    def __init__(self, scope_manager, expression_evaluator, procedure_manager):
        # Композиция: создаем экземпляры компонентов и сохраняем их как атрибуты
