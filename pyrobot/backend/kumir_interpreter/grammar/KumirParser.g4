...
// Использует algorithmName: ID+ и исправленное правило lvalue.
// Ожидается прохождение 57 тестов, 3 ошибки для имен с "И"/числами.
 ...
 */
/**
 * algorithmName: Имя алгоритма. Используется в заголовке и для проверки в конце.
 * Установлено в ID+ для избежания неоднозначности и корректного парсинга
 * заголовков функций/процедур. Обрабатывает одно или несколько слов-идентификаторов.
 */
algorithmName: ID+ ;
 ...
 * Стандартная структура, использующая algorithmName: ID+
 */
algorithmHeader
 ...
 * ИСПРАВЛЕНО: Разрешен RETURN_VALUE ('знач').
 */
lvalue
