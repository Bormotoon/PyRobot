...
def length(text):
    ...
    """    # Приводим входной параметр к строковому типу и используем функцию len
...
def char_code(c):
    ...
    Возвращаемое значение:
      int: Порядковый номер символа в кодировке CP-1251.    Исключения:
      ValueError: Если входной аргумент не является строкой длины 1
                  или при ошибке кодирования.
    """
    s = str(c)
    if len(s) != 1:
    ...
    try:
        ...
        raise ValueError(f"Error encoding character '{s}' in CP-1251: {e}")    # Возвращаем первый байт, который и является кодом символа
    # в данной кодировке
    return encoded[0]
...
def unicode_code(c):
    ...
    s = str(c)
    if len(s) != 1:
        raise ValueError("Function 'unicode_code' expects a string of length 1")    # Используем встроенную функцию ord для получения Unicode-кода символа.
    return ord(s)
...
def char(n):
    ...
    Возвращаемое значение:
      str: Символ, соответствующий заданному коду.    Исключения:
      ValueError: Если число n не может быть корректно преобразовано
                  в символ по CP-1251.
    """
    try:
        n_int = int(n)        # Создаем объект bytes из одного байта и декодируем его
        # с использованием кодировки CP-1251.
        b = bytes([n_int])
