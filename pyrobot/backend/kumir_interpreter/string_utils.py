...
Реализованы функции для преобразования регистра, поиска подстроки,
вставки, замены и удаления символов в строках.
Функции соответствуют требованиям языка KUMIR и используются,
например, в безопасном вычислении выражений.
"""
...
def position(substring, text):
    ...
    Параметры:
      ...
      text (str): Текст, в котором выполняется поиск.    Возвращаемое значение:
      int: Позиция первого вхождения подстроки (начиная с 1) или 0,
           если подстрока не найдена.
    """
    # Преобразуем оба аргумента в строки
    s = str(text)
    sub = str(substring)    # Используем метод find для поиска подстроки; возвращает индекс (начиная с 0)
    # или -1, если не найдено
    idx = s.find(sub)
    # Если индекс не равен -1, прибавляем 1 для перехода к 1-based индексированию,
    # иначе возвращаем 0
...
def position_after(start, substring, text):
    ...
    Параметры:
      ...
      substring (str): Искомая подстрока.
      text (str): Текст, в котором выполняется поиск.    Возвращаемое значение:
      int: Позиция первого вхождения подстроки (начиная с 1)
           после указанной позиции или 0, если подстрока не найдена.

    Исключения:
      ValueError: Если параметр start не является целым числом
    ...
    s = str(text)
    sub = str(substring)    # Проверяем, что стартовая позиция находится в допустимом диапазоне
    if s_start < 1 or s_start > len(s) + 1:
        raise ValueError("position_after: 'start' is out of allowed range")
    # Метод find принимает второй аргумент – позицию, с которой начинать поиск
    # (0-based)
...
def replace(text, old_sub, new_sub, every):
    ...
    Если параметр every (строка) равен "да" (без учета регистра) –
    заменяются все вхождения, если "нет" – заменяется только первое вхождение.

    Параметры:
      text (str): Исходный текст.
      old_sub (str): Подстрока, которую нужно заменить.
      new_sub (str): Подстрока-замена.
      every (str): Параметр, определяющий, заменять все вхождения ("да")
                   или только первое ("нет").
    ...
    s = str(text)
    old = str(old_sub)
    new = str(new_sub)    # Приводим параметр every к нижнему регистру и убираем пробелы для сравнения
...
def delete(text, start, count):
    """
    Удаляет указанное количество символов из текста,
    начиная с заданной позиции (1-based).

    Если сумма start и count превышает длину текста,
    ...
    Исключения:
      ValueError: Если start или count не являются целыми числами
                  или start выходит за допустимый диапазон.
    """
    s = str(text)
    try:
        s_start = int(start)
    ...
    if s_start < 1 or s_start > len(s) + 1:
        raise ValueError("delete: 'start' is out of allowed range")    # Если start+count превышает длину текста, возвращаем строку до start-1
