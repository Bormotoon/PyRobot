...
export const HINTS = {
	initial: ["Добро пожаловать в симулятор! Используйте кнопки слева, чтобы управлять роботом.", "Стартуем! Слева — панель управления: двигайте робота, рисуйте стены и ставьте маркеры.", "Привет! Для перемещения робота и редактирования поля нажимайте соответствующие кнопки слева.", "Вы в игре! Чтобы начать, двигайте робота или войдите в режим редактирования.", "Здравствуйте! Это поле управления: кнопки слева, а здесь — ваше игровое пространство.", "Симулятор запущен! Попробуйте покликать на стрелки или войти в режим редактирования."],
...
/**
 * getHint(action, editMode)
 * Возвращает одну из строк HINTS[action], исключая повтор той же самой строки дважды подряд.
 * @param {string} action - ключ действия, например "moveRobotUp"
 * @param {boolean} editMode - флаг режима рисования (здесь можно расширять логику).
 * @returns {string} одна из строк массива HINTS[action], или пустая строка, если не найдено.
 */
export function getHint(action, editMode = false) {
	// Проверяем, есть ли массив подсказок для данного action
	if (!HINTS[action] || HINTS[action].length === 0) {
		return "";
	}

	const hintsArray = HINTS[action];
	// Если только один вариант, возвращаем сразу его
	...
	// Если массив из 2 элементов, то при совпадении просто берём другой
	if (hintsArray.length === 2) {
		while (hintsArray[randomIndex] === lastUsed) {
			randomIndex = (randomIndex + 1) % 2;
		}
	} else {
		// Если вариантов больше 2, крутим цикл, пока не найдём отличную от последней
		while (hintsArray[randomIndex] === lastUsed) {
	...
	// Запоминаем выбранную строку как "последнюю" для этого action
